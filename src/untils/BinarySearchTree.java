...
public class BinarySearchTree<ContentType extends ComparableContent<ContentType>> {
	...
	private int tiefeLinkerTeilbaum;
	private int tiefeRechterTeilbaum;
	private int tiefe;
	private int balanceFactor;



	public int tiefeBerechnen(){
		tiefeLinkerTeilbaum=0;
		if (node.left!=null){
			tiefeLinkerTeilbaum=node.left.tiefeBerechnen();
		}
		tiefeRechterTeilbaum=0;
		if (node.right!=null){
			tiefeRechterTeilbaum=node.right.tiefeBerechnen();
		}
		if (tiefeLinkerTeilbaum>=tiefeLinkerTeilbaum){
			tiefe=tiefeLinkerTeilbaum+1;
		}else{
			tiefe=tiefeRechterTeilbaum+1;
		}
		return tiefe;
	}

	public void BalanceFactorBerechnen(){
		balanceFactor=tiefeRechterTeilbaum-tiefeLinkerTeilbaum;
	}

	public void Balanciren(){
		if (balanceFactor<-1){
			if (node.left.balanceFactor<-1){
				Rechtsrotieren();
			}else{
				node.left.Linksrotieren();
				Rechtsrotieren();
			}
		}
		if (balanceFactor>1){
			if (node.right.balanceFactor<1){
				...
				node.right.Rechtsrotieren();
				Linksrotieren();
			}
		}
		node.left.Balanciren();
		node.right.Balanciren();
	}

	public void Rechtsrotieren(){
		BinarySearchTree speicher;
		speicher=node.left;
		node.left=node.left.getRightTree();
		speicher.node.right=this;
	}

	public void Linksrotieren(){}
